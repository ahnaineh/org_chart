...
class QuadTree<E> {
  ...
  void _collectNodesInRange(
    ...
    final circleBounds = Rect.fromLTRB(circleLeft, circleTop, circleRight, circleBottom);
    ...
    if (isLeaf) {
      for (final node in _nodes!) {
        final nodeCenter = node.position + Offset(nodeSize.width / 2, nodeSize.height / 2);
