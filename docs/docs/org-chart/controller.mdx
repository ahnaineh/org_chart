...
| `toSetter` (optional) | `void Function(E, String?)` | Function to update an item's parent/manager ID |
...
| Action | Description |
|--------|-------------|
| `ActionOnNodeRemoval.unlink` | Removes node and sets its subordinates' manager to `null` |
| `ActionOnNodeRemoval.connectToParent` | Connects subordinates to the removed node's own manager |
