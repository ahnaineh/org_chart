...
|-----------|------|----------|---------|-------------|
| `items` | `List<T>` | ✅ | - | The list of data items to display in the chart |
| `idProvider` | `String Function(T)` | ✅ | - | Function that returns a unique ID for each item |
| `toProvider` | `String? Function(T)` | ✅ | - | Function that returns the parent ID for each item |
| `toSetter` | `T Function(T, String?)` | ❌ | null | Function to create a copy of item with new parent ID |
| `sizeChangeAction` | `SizeChangeAction` | ❌ | `ignore` | What to do when a node's rendered size changes |
| `sizeChangeThreshold` | `double` | ❌ | 0.0 | Minimum size delta before triggering collision handling |
| `preserveManualPositionsOnSizeChange` | `bool` | ❌ | false | Keep manually positioned nodes fixed during size-change collision handling |
| `collisionSettings` | `CollisionAvoidanceSettings` | ❌ | `CollisionAvoidanceSettings()` | Settings for size-change collision handling |
| `spacing` | `double` | ❌ | 20 | Horizontal spacing between nodes |
| `runSpacing` | `double` | ❌ | 50 | Vertical spacing between levels |
| `orientation` | `GraphOrientation` | ❌ | `topToBottom` | Layout orientation |
| `leafColumns` | `int` | ❌ | 2 | Number of columns for leaf node arrangement |
...
Next: Learn about the [OrgChart Widget →](/docs/3.0.0-orgchart/03-widget)
