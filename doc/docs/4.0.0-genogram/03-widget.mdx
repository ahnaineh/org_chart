...
Genogram<T>({
  ...
  EdgeStyle Function(EdgeInfo<T>)? edgeStyleProvider,
  Widget Function(EdgeInfo<T>)? edgeLabelBuilder,
  EdgeLabelConfig edgeLabelConfig = const EdgeLabelConfig(),
...
| `edgeStyleProvider` | `Function?` | null | Per-edge styling (opacity, color, width) |
| `edgeLabelBuilder` | `Function?` | null | Builds widget labels for edges |
| `edgeLabelConfig` | `EdgeLabelConfig` | default | Label placement configuration |
...
Genogram(
  edgeConfig: GenogramEdgeConfig(
    ...
    // Anchor distance from spouse for label/child edge
    marriageAnchorDistance: 16.0,
  ),
)
```
